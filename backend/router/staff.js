...
router.get("/staffsales/:type", (req, res) => {
  const { type } = req.params;

  let condition = "";

  if (type === "daily") {
    condition = "DATE(bill.date) = CURDATE()";
  } else if (type === "monthly") {
    condition = "MONTH(bill.date) = MONTH(CURDATE()) AND YEAR(bill.date) = YEAR(CURDATE())";
  } else if (type === "yearly") {
    condition = "YEAR(bill.date) = YEAR(CURDATE())";
  }

  const sql = `
    SELECT 
      user.name,
      SUM(bill.Amount) AS sales,
      SUM(bill.quantity) AS totQuantity
    FROM bill
    JOIN user ON bill.Stid = user.id
    WHERE user.Action='staff' AND ${condition}
    GROUP BY user.id
    ORDER BY sales DESC
    LIMIT 5
  `;
